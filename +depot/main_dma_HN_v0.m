...
load('D:\Hamed\CND\PhD\Experiments\PFA-RH122\PFA results\SMPS\February\FEB22-24\Post-processed\FEB22_ET_LAL_HR_WR.mat')

d = logspace(log10(20), log10(1.2e3), 500)';  % reconstruction points
% d_star = logspace(log10(13.1), log10(200), 114)';  % mobility setpoints
% 763.5
d_star = (dn_raw(1,:))';

prop = tfer.prop_dma_HN_DIST;
...
% [~, ~, x0] = tools.gen_data(A, d, mu, s, w, d_star);
% [b, Lb, x0] = tools.gen_data(A, d, 75, 1.1, w_d, d_star);

b = (mean(dn_raw(2:4,:)))';
Lb = bsxfun(@rdivide, eye(length(b)), (std(dn_raw(2:4,:)))');
...
subplot(2, 3, 1);
tools.plotci(d, x_tk1, Gpo_tk1);
title('Tikhonov (1st)');

subplot(2, 3, 2);
tools.plotci(d, x_tk2, Gpo_tk2);
title('Tikhonov (2nd)');

subplot(2, 3, 3);
tools.plotci(d, x_tk, Gpo_tk);
title('Tikhonov (2nd, two-step)');

subplot(2, 3, 4);
tools.plotci(d, x_ed, Gpo_ed);
title('Exponential distance');

subplot(2, 3, 5);
tools.plotci(d, x_two, []);
title('Twomey');
hold on;
plot(d, xi, 'b--');
hold off;
